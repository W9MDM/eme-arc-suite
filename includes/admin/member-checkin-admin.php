...
function eme_member_checkin_report_page() {
    ...
    $query = "
        SELECT c.id, c.checkin_time, p.firstname, p.lastname, p.city, p.state_code, a.answer AS callsign,
               m.membership_id, m.status, m.end_date, mem.name AS membership_name
                ...
                <div id="tab-<?php echo esc_attr($date); ?>" class="tab-content" style="<?php echo $first ? '' : 'display: none;'; ?>">
                    ...
                    <div class="tablenav top">
                        <div class="tablenav-pages">
                            <span class="displaying-num"><?php printf(esc_html__('%d items', 'eme-member-checkin'), $total); ?></span>
                            <span class="pagination-links">
                                <?php if ($current_page > 1): ?>
                                    <a class="first-page button" href="<?php echo esc_url(add_query_arg(["{$date}_page" => 1, "{$date}_per_page" => $per_page])); ?>">«</a>
                                    <a class="prev-page button" href="<?php echo esc_url(add_query_arg(["{$date}_page" => $current_page - 1, "{$date}_per_page" => $per_page])); ?>">‹</a>
                                <?php else: ?>
                                    <span class="tablenav-pages-navspan button disabled">«</span>
                                    <span class="tablenav-pages-navspan button disabled">‹</span>
                                <?php endif; ?>
                                <span class="paging-input">
                                    <?php printf(esc_html__('Page %d of %d', 'eme-member-checkin'), $current_page, $total_pages); ?>
                                </span>
                                <?php if ($current_page < $total_pages): ?>
                                    <a class="next-page button" href="<?php echo esc_url(add_query_arg(["{$date}_page" => $current_page + 1, "{$date}_per_page" => $per_page])); ?>">›</a>
                                    <a class="last-page button" href="<?php echo esc_url(add_query_arg(["{$date}_page" => $total_pages, "{$date}_per_page" => $per_page])); ?>">»</a>
                                <?php else: ?>
                                    <span class="tablenav-pages-navspan button disabled">›</span>
                                    <span class="tablenav-pages-navspan button disabled">»</span>
                                <?php endif; ?>
                        ...
                        <div class="alignright">
                            <label for="<?php echo esc_attr($date); ?>_per_page" class="screen-reader-text"><?php esc_html_e('Entries per page', 'eme-member-checkin'); ?></label>
                            <select name="<?php echo esc_attr($date); ?>_per_page" onchange="window.location.href=this.value;">
                                <?php foreach ($per_page_options as $option): ?>
                                    <option value="<?php echo esc_url(add_query_arg(["{$date}_per_page" => $option, "{$date}_page" => 1])); ?>" <?php selected($per_page, $option); ?>><?php echo esc_html($option); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
